...
def train_and_swap(fn, old, tmp, verbose=False):
    if os.path.exists(fn):
        ...
        old_model.load(fn)
        have_cache_data = True
    else:
        old_model = None
        have_cache_data = False

    new_model = train_and_save_model(tmp, verbose=verbose,have_cache_data=have_cache_data)
    max_retries = 5
    current_retry = 1
...
def train_and_save_model(fn, verbose=True, emphasize_experiments=0,have_cache_data=True):
    ...
    reg = model.BaoRegression(have_cache_data=have_cache_data, verbose=verbose)
    reg.fit(x, y)
    reg.save(fn)
    return reg
